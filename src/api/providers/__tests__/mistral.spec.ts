...
// npx vitest run src/api/providers/__tests__/mistral.spec.ts
...
describe("MistralHandler", () => {
	...
	describe("FIM support", () => {
		...
		it("supportsFim returns true for codestral-2405", () => {
			const handler = new MistralHandler({
				...mockOptions,
		...
		it("supportsFim returns true when no model is specified (defaults to codestral-latest)", () => {
			const handler = new MistralHandler({
				mistralApiKey: "test-api-key",
			})
		...
		it("completeFim aggregates all chunks", async () => {
			const handler = new MistralHandler({
				...mockOptions,
				apiModelId: "codestral-latest",
			})
		...
		it("streamFim uses correct endpoint for codestral models", async () => {
			const handler = new MistralHandler({
				...mockOptions,
				apiModelId: "codestral-latest",
			})
		...
		it("streamFim uses custom codestral URL when provided", async () => {
			const handler = new MistralHandler({
				...mockOptions,
				apiModelId: "codestral-latest",
